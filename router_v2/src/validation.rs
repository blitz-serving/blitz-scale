...
impl Validation {
    ...
    async fn validate_input(
    ...
    ) -> Result<(String, usize, u32, Vec<u32>), ValidationError> {
        // If we have a fast tokenizer
        if let Some(sender) = &self.sender {
            ...
            // Unwrap is safe here
            let (inputs, input_length, input_tokens) = response_receiver.await.unwrap()?;
            ...
            Ok((inputs, input_length, max_new_tokens, input_tokens))
        }
        // Return inputs without validation
            ...
            Ok((inputs, input_length, max_new_tokens, vec![]))
        }
    }
    ...
    pub(crate) async fn validate(
        ...
        // Validate inputs
        let (inputs, input_length, max_new_tokens, input_tokens) = self
        ...
        Ok(ValidGenerateRequest {
            request_id: RID_GENERATOR.fetch_add(1, std::sync::atomic::Ordering::SeqCst),
            inputs,
            decoder_input_details,
...
fn prepare_input(
    ...
    truncate: Option<usize>,
    tokenizer: &Tokenizer,
) -> Result<(String, usize, Vec<u32>), ValidationError> {
    ...
    Ok((inputs, input_length, input_tokens))
}

type TokenizerRequest = (
    (String, Option<usize>),
    oneshot::Sender<Result<(String, usize, Vec<u32>), ValidationError>>,
    Span,
);
